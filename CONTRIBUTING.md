...
3. Under **Rich Presence > Art Assets**, upload your images and note their asset keys. The large image is picked from the detected agent, so upload one per agent named `logo-chatgpt`, `logo-claude`, `logo-cursor`, `logo-grok`, and `logo-antigravity`. The small image is still configurable in Settings and defaults to `discord-presence-icon`.
...
Once Discord is running and a supported coding-agent session is active, the presence shows up on your profile. It clears again about a minute after the last Claude / Codex / Grok / Antigravity transcript activity (Cursor clears when the hook turn ends) or when you quit the app.
...
- The app also needs to read `~/.claude/projects/`, `~/.gemini/antigravity-cli/`, etc., which a sandbox would block.
...
Components plus the SwiftUI app shell:
...
- `AntigravitySession.swift` - watches `~/.gemini/antigravity-cli/` (`brain/`, `presence/`, `history.jsonl`) with `FSEvents` and fixed-depth transcript indexing. Activity comes from transcript events, presence lock mtimes, and history stamps. Elapsed time is today's working duration (idle gaps excluded).
- `PresenceController.swift` - observes Claude, Codex, Cursor, Grok, and Antigravity sessions, selects the most recently active enabled agent, builds the activity from your settings, debounces updates (at most roughly once every few seconds, and only when the content actually changes), and clears on idle or quit. Presence idle is hardcoded at 60 seconds. Discord elapsed is the sum of enabled agents' daily totals; popover rows stay per-agent.
...
- A Claude, Codex, Grok, or Antigravity session is considered active if its transcript or session files were modified within the last 60 seconds. A Grok TUI that is open but idle does not count; an open turn that has not yet `turn_ended` stays live. Cursor is active only while a hook turn is open.
- For Claude Code, token totals cover the current local calendar day and reset at midnight. Elapsed time is today's sum of working gaps for every agent (Claude, Codex, Cursor, Grok, Antigravity), so a 1pm–2pm block plus a 5pm–6pm block shows as two hours. After idle the row keeps that number in gray. Discord elapsed adds those daily totals together. Codex, Grok, and Antigravity report the current session's context tokens instead.
