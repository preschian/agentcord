...
The [release workflow](.github/workflows/release.yml) builds both platforms and attaches `AgentCord.dmg`, `agentcord.exe`, and the GPUI prototype `agentcord-gpui.exe` to the release. You can also trigger it manually from the Actions tab with a tag name.
...
- `ClaudeSession.swift` - watches `~/.claude/projects/` with `FSEvents` plus a periodic re-scan. The most recently modified `.jsonl` transcript defines the active session (project + model). Tokens are summed across transcripts touched on the current local calendar day; elapsed time is the summed working duration across transcripts that touched the last 24 hours (idle gaps excluded), matching Grok / Codex / Cursor. Per-file parse results are memoized (keyed by modification time and the day boundary) so each scan only re-reads files that actually changed. Parsing is defensive: malformed or unexpected lines are skipped, never fatal.
- `CodexSession.swift` - first connects to an existing Codex App Server daemon and reads official runtime thread status. Standalone CLI processes do not share that runtime, so it falls back to watching `~/.codex/sessions/` transcripts. Transcript data enriches both paths with cwd, model, timestamps, and token totals. Elapsed time is the summed working duration across transcripts that touched the last 24 hours (idle gaps excluded).
- `GrokSession.swift` - watches `~/.grok/active_sessions.json` and per-session `summary.json` / `signals.json` under `~/.grok/sessions/`. Activity comes from `last_active_at` and event-log mtimes (so an idle TUI is idle, while a long tool run stays active). Summary timestamps and context-window signals enrich project, model, and token fields. Falls back briefly to the last-known session during the idle grace period after a quit. Elapsed time is the summed working duration across sessions that touched the last 24 hours (idle gaps excluded), so a 1pm–2pm block plus a 5pm–6pm block shows as two hours.
...
- A Claude, Codex, or Grok session is considered active if its transcript or session files were modified within the idle window (default 5 min, configurable 5-30 min in 5-minute steps). A Grok TUI that is open but idle does not count.
- For Claude Code, token totals cover the current local calendar day and reset at midnight. Elapsed time is a rolling 24-hour sum of working gaps for every agent (Claude, Codex, Cursor, Grok), so a 1pm–2pm block plus a 5pm–6pm block shows as two hours. Codex and Grok report the current session's context tokens instead.
