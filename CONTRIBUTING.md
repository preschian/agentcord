...
- `GrokSession.swift` - watches `~/.grok/active_sessions.json` and per-session `summary.json` / `signals.json` under `~/.grok/sessions/`. Activity comes from `last_active_at` and event-log mtimes (so an idle TUI is idle, while a long tool run stays active). Summary timestamps and context-window signals enrich project, model, and token fields. Falls back briefly to the last-known session during the idle grace period after a quit.
