...
public sealed class AntigravitySession : IDisposable
{
    ...
    public bool IsLinked => Directory.Exists(_baseDir);
    public bool IsAuthenticated => !string.IsNullOrEmpty(AccountEmail) || ReadGoogleAccountEmail() is not null || CheckOAuthCreds();
    ...
    public static string? ReadGoogleAccountEmail()
    {
        try
        {
            var userProfile = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile);
            var path = Path.Combine(userProfile, ".gemini", "google_accounts.json");
            if (File.Exists(path))
            {
                using var doc = JsonDocument.Parse(File.ReadAllText(path));
                if (doc.RootElement.TryGetProperty("active", out var active) && active.ValueKind == JsonValueKind.String)
                {
                    var email = active.GetString();
    ...
    public static bool CheckOAuthCreds()
    {
        try
        {
            var userProfile = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile);
            var oauth = Path.Combine(userProfile, ".gemini", "oauth_creds.json");
            return File.Exists(oauth);
        }
        catch { return false; }
    }
    ...
    {
        if (!Directory.Exists(_baseDir)) return default;
    ...
    {
        if (AccountEmail is null && ReadGoogleAccountEmail() is { } googleEmail)
